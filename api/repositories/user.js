class User {
    ...
    async save(user) {
        try {
            ...
            const hash = this.crypto.createHmac('sha256', this.cfg.passSecret)
    ...
    async updatePassword(user) {
        try {
            ...
            if (user.password !== account.password) throw new Error('Password did not match')

            const newHash = this.crypto.createHmac('sha256', this.cfg.passSecret)
                .update(user.newPassword)
                .digest('hex')

            user.newPassword = newHash
            return await this.userSchema.findOneAndUpdate(
                { _id: user.id }, { $set: { password: user.newPassword } }, { new: true }
            )
        } catch (error) {
            throw error
        }
    ...
    generateToken(account) {
        let payload = {
            ...
            exp: this.moment().add(24, 'hours').unix()
        }
